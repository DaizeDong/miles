...
def load_logits_data(filepath: str, use_weights_only: bool = False,
    ...
    try:
        ...
        _ = file_size_mb
        data = load_router_replay_pt_file(
            filepath,
...
def load_router_replay_bundle(filepath: str, use_weights_only: bool = False,
                              target_device: str = 'cpu') -> Dict:
    try:
        return load_router_replay_artifact_bundle(
            filepath,
            target_device=target_device,
            use_weights_only=use_weights_only,
        )
    except Exception as e:
        print(f"  ❌ 加载artifact bundle {os.path.basename(filepath)} 出错: {e}")
        return None


def print_predictive_bundle_summary(bundle: Dict, label: str) -> None:
    main_data = bundle["main"]
    predictive_bias = main_data.get("predictive_bias", {})
    predictive_metrics = bundle.get("predictive_metrics")
    predictive_metric_tensors = bundle.get("predictive_metric_tensors")

    bias_layer_count = len(predictive_bias) if isinstance(predictive_bias, dict) else 0
    metrics_state = "present" if predictive_metrics else "missing"
    tensors_state = "present" if predictive_metric_tensors else "missing"
    print(
        f"{label} predictive: bias_layers={bias_layer_count}, "
        f"metrics_sidecar={metrics_state}, metric_tensors={tensors_state}"
    )
    if predictive_metrics and predictive_metrics.get("aggregates"):
        print(f"{label} predictive aggregates: {predictive_metrics['aggregates']}")
...
def analyze_step_group(log_prob_file: str, training_files_dict: Dict[str, str],
    ...
    t_start = time.time()
    print("\n[1/5] 加载 log_prob 文件...")
    log_prob_bundle = load_router_replay_bundle(log_prob_file)
    if not log_prob_bundle:
        print("  ❌ log_prob 加载失败")
        return 'failed'

    log_prob_data = log_prob_bundle["main"]
    ...
    print_predictive_bundle_summary(log_prob_bundle, "  ✓ log_prob")
    ...
    for mini in sorted(training_files_dict.keys(), key=lambda x: int(x)):
        ...
        training_bundle = load_router_replay_bundle(training_file)
        if training_bundle:
            training_data_list.append((mini, training_bundle["main"]))
            print(f"  ✓ mini{mini}: {os.path.basename(training_file)}")
            print_predictive_bundle_summary(training_bundle, f"    mini{mini}")
