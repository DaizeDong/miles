...
def test_save_hf_model_raw_source_uses_rollout_sync_named_tensors():
    source = Path(
        "/mnt/weka/home/hongyi.wang/workspace/rlhf/miles/miles/backends/megatron_utils/model.py"
    ).read_text()

    assert "rollout_sync_named_params_and_buffers(args, model, convert_to_global_name=True)" in source
    assert "include_predictive_params=True" not in source
