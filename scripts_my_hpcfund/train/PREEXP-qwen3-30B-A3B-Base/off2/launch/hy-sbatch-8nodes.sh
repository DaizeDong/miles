#!/bin/bash
# Verl-aligned 8-node baseline-style startup script.
#SBATCH --job-name=miles-qwen30b-8nodes
#SBATCH --nodes=8
...
export RUN_POSTFIX="${RUN_POSTFIX:-${OFF_POLICY_LABEL}-8nodes-baseline}"
###########################################
...
export CKPT_MONITOR_SCRIPT_IN_CONTAINER="${CKPT_MONITOR_SCRIPT_IN_CONTAINER:-$(resolve_container_repo_path "scripts_my_hpcfund/train/ckpt_monitor_miles.sh")}"
...
export PREDICTIVE_REPLAY_ARGS_SCRIPT_IN_CONTAINER="${PREDICTIVE_REPLAY_ARGS_SCRIPT_IN_CONTAINER:-/root/miles/scripts_my_hpcfund/train/PREEXP-qwen3-30B-A3B-Base/off2/launch/_predictive_replay_args.sh}"
export CHAT_TEMPLATE_PATH="${CHAT_TEMPLATE_PATH:-}"
export APPLY_CHAT_TEMPLATE_KWARGS="${APPLY_CHAT_TEMPLATE_KWARGS:-}"
export RUNTIME_PYTHONPATH="${RUNTIME_PYTHONPATH:-/work1/hwang/dzdong/python_extras:/root/miles:/app/Megatron-LM/}"
...
if [ "${THIS_NODE}" = "${HEAD_NODE}" ]; then
  export PYTHONPATH="/work1/hwang/dzdong/python_extras:/root/miles:/app/Megatron-LM/${PYTHONPATH:+:${PYTHONPATH}}"
  ...
  echo "[HEAD] waiting for ray dashboard at ${HEAD_IP}:${DASHBOARD_PORT}..."
  for _i in $(seq 1 120); do
    _code=$(curl -s -m 2 -o /dev/null -w "%{http_code}" "http://${HEAD_IP}:${DASHBOARD_PORT}/api/version" 2>/dev/null || true)
    if [ "${_code}" = "200" ]; then
  ...
  if [ "${SLURM_NNODES}" -eq 1 ]; then
    echo "[HEAD] single-node run; ray head already has ${TARGET_GPUS} GPUs, skipping wait-for-workers loop"
    AVAILABLE_GPUS="${TARGET_GPUS}"
  else
  while true; do
...
PY
  ...
  set +e
  # Bypass `ray job submit` (dashboard job-submission agent hangs on
  # this cluster — 504 after 300s). The Ray worker processes are
  # spawned by raylet (which already inherited our exported env),
  # so we only need to ensure the runtime-env keys are exported.
  export PYTHONPATH="/work1/hwang/dzdong/python_extras:/root/miles:/app/Megatron-LM/${PYTHONPATH:+:${PYTHONPATH}}"
  export CUDA_DEVICE_MAX_CONNECTIONS=1
  export NCCL_NVLS_ENABLE="${HAS_NVLINK}"
  # 309779 hung 9.5h after `Connected to Ray cluster` with no
  # output. python3 default buffering hides exactly the kind of
  # log we need (placement-group, sglang boot, actor init). Run
  # unbuffered; also widen ray verbosity so worker-side errors
  # (e.g. raylet registration failures) reach our redirected log.
